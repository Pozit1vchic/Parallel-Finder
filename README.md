...
В блоке «Состав анализа» можно выбрать, какие временные окна строить:

| Режим | Что сравнивается |
| --- | --- |
| **Движение** | окна около 2,5 секунды с обязательным движением суставов; статика отбрасывается |
| **Статические кадры** | независимые последовательности кадров около 2 секунд, без требования движения |
| **Клипы 4 с** | более длинные 4-секундные окна для монтажных клипов |
| **Движение + кадры** | оба независимых прохода; статические и движущиеся окна не смешиваются |

В каждом режиме одна картинка не является результатом: окно должно содержать
много разных временных сэмплов и устойчивую серию совпадающих кадров.
...
Проверочный набор хранится только в локальном checkout разработчика и намеренно
игнорируется Git. Если папка `tests/` есть на вашей машине, включите её явно:
...
Это не маркетинговый релиз. Локальные unit/UI/smoke-тесты подтверждают сборку
и базовые контракты, но не гарантируют одинаковое качество на каждом фильме,
драйвере и модели. Перед публикацией portable-версии нужно отдельно проверить:
