...
TEST(SceneContext, NormalizedBackgroundAndMissingSamples)
{
    std::vector<std::uint8_t> rgba(32 * 32 * 4, 255);
    const std::vector<pfcore::SceneSample> samples = {{1.0, 32, 32, rgba}};
    const auto context = pfcore::sceneContext(samples, 0.0, 2.0);
    ASSERT_EQ(context.size(), 44U);
    double sum = 0;
    for (auto value : context) sum += value;
    EXPECT_NEAR(sum, 1.0, 1e-6);
    EXPECT_TRUE(pfcore::sceneContext(samples, 3.0, 4.0).empty());
    const std::vector<pfcore::SceneSample> malformed = {{1.0, 32, 32, {}}};
    EXPECT_TRUE(pfcore::sceneContext(malformed, 0.0, 2.0).empty());
}
