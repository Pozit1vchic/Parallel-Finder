...
TEST(DominantIdentity, ClothingBridgeCannotMergeConflictingFaces)
{
    using pfcore::IdentitySummary;
    const std::vector<IdentitySummary> people = {
        {{1,0}, {1,0}, 1,1,20,1}, // lead
        {{1,0}, {}, 1,0,2,1},     // similar uniform, face unavailable
        {{1,0}, {0,1}, 1,1,8,1},  // other person: matching clothes, conflicting face
        {{0,1}, {1,0}, 1,1,5,1},  // lead wearing different clothes
        {{1,0}, {0,1}, 1,1,6,1}   // valid repeat of the secondary person
    };
    const auto selected = pfcore::selectDominantIdentities(people);
    ASSERT_EQ(selected.size(), 5U);
    EXPECT_TRUE(selected[0]); EXPECT_TRUE(selected[3]);
    EXPECT_FALSE(selected[2]); EXPECT_FALSE(selected[4]);
}

TEST(DominantIdentity, FaceSimilarityIsNotTransitive)
{
    // A agrees with B and B with C, but A explicitly disagrees with C.
    const std::vector<pfcore::IdentitySummary> people = {
        {{}, {1,0}, 0,1,20,1},
        {{}, {0.8F,0.6F}, 0,1,5,1},
        {{}, {0,1}, 0,1,2,1}
    };
    const auto selected = pfcore::selectDominantIdentities(people);
    EXPECT_TRUE(selected[0]); EXPECT_TRUE(selected[1]); EXPECT_FALSE(selected[2]);
    EXPECT_TRUE(pfcore::selectDominantIdentities({}).empty());
}

TEST(DominantIdentity, MissingEvidenceKeepsOnlyLongestTrack)
{
    const auto selected = pfcore::selectDominantIdentities({
        {{},{},0,0,1,1}, {{},{},0,0,5,1}, {{},{},0,0,2,1}
    });
    EXPECT_FALSE(selected[0]); EXPECT_TRUE(selected[1]); EXPECT_FALSE(selected[2]);
}
...
TEST(MotionMatcher, IdenticalCloseUpsDoNotRequireInvisibleLegs)
{
    pfcore::MotionWindow a; a.sourceId = "a"; a.staticFrameSet = true;
    for (int i = 0; i < 12; ++i) {
        std::vector<pfcore::Keypoint> points(17, {0, 0, 0});
        points[0] = {0, -0.5, 1};
        points[1] = {-0.08, -0.55, 1}; points[2] = {0.08, -0.55, 1};
        points[3] = {-0.16, -0.5, 1}; points[4] = {0.16, -0.5, 1};
        points[5] = {-0.5, 0, 1}; points[6] = {0.5, 0, 1};
        a.frames.push_back({i / 6.0, points});
    }
    auto b = a; b.sourceId = "b";
    for (auto& f : b.frames) {
        f.timestampSeconds += 10;
        for (auto& p : f.keypoints) { p.x = 2 * p.x + 4; p.y = 2 * p.y + 7; }
    }
    pfcore::MotionMatcherParams params; params.allowStaticFrames = true;
    EXPECT_GT(pfcore::MotionMatcher(params).compare(a, b).similarity, 0.9);
    auto ranked = pfcore::MotionMatcher(params).findAllPairs({a, b});
    pfcore::MotionRanker::rank(ranked, {a, b});
    ASSERT_EQ(ranked.size(), 1U);
    EXPECT_EQ(ranked.front().gestureLabel, "static");
    EXPECT_EQ(ranked.front().directionLabel, "static");
    // Face-only versus head-and-shoulders must use the same head scale and
    // still preserve orientation. Independent identity evidence is mandatory.
    a.faceEmbedding = b.faceEmbedding = {1, 0};
    a.faceConfidence = b.faceConfidence = 1;
    for (auto& f : a.frames) for (int j = 5; j < 17; ++j) f.keypoints[j].confidence = 0;
    EXPECT_GT(pfcore::MotionMatcher(params).compare(a, b).similarity, 0.9);
    auto rotated = b;
    for (auto& f : rotated.frames) for (auto& p : f.keypoints) {
        const double oldX = p.x; p.x = -p.y; p.y = oldX;
    }
    EXPECT_DOUBLE_EQ(pfcore::MotionMatcher(params).compare(a, rotated).similarity, 0.0);
    auto wrongIdentity = b; wrongIdentity.faceEmbedding = {0, 1};
    params.requireAppearance = true;
    EXPECT_DOUBLE_EQ(pfcore::MotionMatcher(params).compare(a, wrongIdentity).similarity, 0.0);
    // Fewer than six shared joints still cannot substantiate a pose match.
    for (auto& f : b.frames) for (int j = 3; j < 17; ++j) f.keypoints[j].confidence = 0;
...
TEST(MotionMatcher, FaceIdentitySurvivesChangedClothes)
{
    auto left = gestureWindow("a", 0, false), right = gestureWindow("b", 8, false);
    left.faceEmbedding = right.faceEmbedding = {1,0,0};
    left.faceConfidence = right.faceConfidence = 1;
    left.appearanceEmbedding = {1,0,0}; right.appearanceEmbedding = {0,1,0};
    left.appearanceConfidence = right.appearanceConfidence = 1;
    pfcore::MotionMatcherParams params; params.requireAppearance = true;
    const auto matches = pfcore::MotionMatcher(params).findAllPairs({left,right});
    ASSERT_EQ(matches.size(),1U);
    EXPECT_TRUE(matches[0].faceVerified);
}

TEST(MotionMatcher, FaceDisagreementVetoesIdenticalClothes)
{
    auto left = gestureWindow("a", 0, false), right = gestureWindow("b", 8, false);
    left.faceEmbedding = {1,0,0}; right.faceEmbedding = {0,1,0};
    left.faceConfidence = right.faceConfidence = 1;
    left.appearanceEmbedding = right.appearanceEmbedding = {1,0,0};
    left.appearanceConfidence = right.appearanceConfidence = 1;
    pfcore::MotionMatcherParams params; params.requireAppearance = true;
    EXPECT_TRUE(pfcore::MotionMatcher(params).findAllPairs({left,right}).empty());
}
...
TEST(MotionMatcher, MaskedCharacterUsesBodyEvidenceWithoutClaimingFaceIdentity)
{
    auto left = gestureWindow("a", 0, false), right = gestureWindow("b", 8, false);
    left.appearanceEmbedding = right.appearanceEmbedding = {1,0,0};
    left.appearanceConfidence = right.appearanceConfidence = 1;
    ...
    const auto matches = pfcore::MotionMatcher(params).findAllPairs({left,right});
    ASSERT_EQ(matches.size(), 1U);
    EXPECT_TRUE(matches[0].appearanceVerified);
    EXPECT_FALSE(matches[0].faceVerified);
    right.appearanceEmbedding = {0,1,0};
    EXPECT_TRUE(pfcore::MotionMatcher(params).findAllPairs({left,right}).empty());
}

TEST(MotionMatcher, NearbyMatchingBackgroundIsSuppressedButDistantRepeatSurvives)
{
    auto left = gestureWindow("a", 0, false), right = gestureWindow("a", 10, false);
    left.sceneContext = right.sceneContext = {0.75F,0.25F};
    pfcore::MotionMatcher matcher;
    EXPECT_EQ(matcher.compare(left,right).similarity, 0);
    right.sourceId = "b";
    EXPECT_GT(matcher.compare(left,right).similarity, 0.78);
    right.sourceId = "a";
    for (auto& frame : right.frames) frame.timestampSeconds += 60;
    EXPECT_GT(matcher.compare(left,right).similarity, 0.78);
}

TEST(DominantIdentity, MaskedLeadDoesNotRequireVisibleFace)
{
    const std::vector<pfcore::IdentitySummary> people = {
        {{1,0}, {}, 1,0,20,1}, {{1,0}, {}, 1,0,12,1},
        {{0,1}, {}, 1,0,10,1}
    };
    const auto selected = pfcore::selectDominantIdentities(people);
