...
В этот проход отдельно проверены границы доверия: сетевые manifest/asset-файлы,
частичные загрузки, tar-распаковка, активный provider marker, FFmpeg-процесс,
кэш и размеры декодируемых кадров. Для них добавлены ограничения и отказ от
непроверенных загрузок; это не заменяет внешний security review релиза.
...
| Один человек сравнивался с другим | **Частично** | Внутри одного файла разные `trackId` теперь жёстко разделены, а OSNet используется для продолжения одного трека через сцену и для разных файлов. Identity benchmark на размеченных людях ещё не выполнен. |
...
- CPU analysis smoke на 30-секундном фрагменте (`1801` кадров) с
  ...
  Без body-ReID новые сборки намеренно не публикуют межперсонажные пары:
  pose-only совпадение не считается доказательством одного человека.
...
## Security-аудит и hardening этого прохода

| Область | Риск | Исправление |
| --- | --- | --- |
| ModelStore | Manifest мог указать путь/имя вне ожидаемого файла или бесконечно большой asset при отсутствующем `sizeBytes`. | Имена только basename, GitHub-only HTTPS, лимит скачивания 4 GiB, лимит manifest, проверка размера и SHA-256 до установки, `.part` остаётся незавершённым до успешной проверки. |
| ModelStore | При недоступном manifest можно было скачать предсказуемый URL без проверки целостности. | Непроверенный fallback удалён: без manifest с size/hash установка отклоняется. |
| ProviderStore | Tar path traversal, symlink/hardlink и установка поверх активного runtime. | Сначала `tar -tf/-tvf` с проверкой путей и типов, распаковка в staging, проверка дерева и `onnxruntime.dll`, затем замена директории с восстановлением backup при ошибке. Используется системный `tar.exe` без shell и с `CREATE_NO_WINDOW`. |
| OrtRuntime | `providers/active.txt` мог подставить произвольный путь. | Marker принимается только для `dml/cuda/tensorrt/cpu`; symlink-каталоги пропускаются. |
| PfCache | Повреждённый заголовок мог запросить огромный `vector::resize`; параллельные записи делили один `.tmp`. | Жёсткий лимит записи 512 MiB и уникальные временные имена с PID/счётчиком. |
| VideoDecoder | Файл с экстремальным разрешением мог вызвать переполнение/большую аллокацию RGBA. | Ограничение 64 Mpix и проверка переполнения размера буфера. |
| ONNX inference | Вредная/сломанная модель могла объявить гигантский input/output tensor или выдать не-embedding ReID. | Общий лимит tensor 64M элементов, ограничение pose input и ReID output (32–4096 элементов). |
| FFmpeg export | Подвисший encoder блокировал worker навсегда; prefix мог использоваться как path traversal в MP4-режиме; bare `ffmpeg` зависел от PATH. | Таймаут 180 с, принудительная остановка, whitelist prefix (`A–Z`, цифры, `.`, `_`, `-`), сначала ищется бинарник рядом с приложением, затем абсолютный путь из PATH. Аргументы передаются напрямую через `QProcess`, без shell. |

## Performance-аудит и фактические ограничения

- Для pose preprocessing таблицы соответствия X/Y вычисляются один раз на
  изображение, а не через две операции деления на каждый пиксель.
- Для fixed-batch моделей staging кадров ограничен бюджетом 128 MiB: на 4K
  входе приложение автоматически уменьшает эффективный batch, чтобы не
  получить скачок памяти в сотни мегабайт. На CPU по-прежнему выбирается
  установленный batch-1 sibling.
- MotionIndex оставляет bounded candidate search, поэтому DTW запускается не
  на всех парах окон. Это оптимизация поиска, а не гарантия точности.
- Кэш анализа версионируется ключом модели/provider/ReID/режима/метаданных
  файла. При изменении этих параметров старый результат не переиспользуется.
- Реальное ускорение CUDA/TensorRT не заявляется без установленного runtime,
  совместимого драйвера и прогона на конкретной видеокарте. Доступный smoke
  test проверяет pipeline, но не throughput и не precision/recall.
